  ...
  var TagCloud = /*#__PURE__*/function () {
    ...
    function TagCloud() {
      ...
      for (var i = 0; i < self.texts.length; i++) {
        ...
        // If the word is already in the counter, increment its count
        if (wordCounts[word]) {
          wordCounts[word]++;
        }
      ...
      for (let wordCount in wordCounts){
        if(wordCounts[wordCount] > maxFreq){
          maxFreq = wordCounts[wordCount];
          maxKey = wordCount;
        }
      }
    ...
    _createClass(TagCloud, [{
      ...
      value: function _createTextItem(text) {
        ...
        if (self.config.useItemInlineStyles) {
          ...
          itemEl.style.fontSize = "".concat(self.fontSizes[text], "px");
          itemEl.style.top = '50%';
